...
<details>
<summary>Hints: baseR/data.table</summary>
<hr>
...
See the example with the "embrace operator", and more
details here: <https://dplyr.tidyverse.org/articles/in-packages.html>
...
### Exercise: writing checks for `plot_xy()`

Instruction: add the following checks to your `filter_islands()` function and
test them in your test script (`tests/test_filter_islands.R`). 
...
<details>
<summary>Hints: unquoted column names with `dplyr::filter`</summary>
<hr>
...
covered by `filter()` anyways. 

In case you wanted to add this, on top of the internal `filter` check,
