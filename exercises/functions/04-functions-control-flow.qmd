...
filter_col <- 'island'
filter_value <- c('TOR', 'COR')

prep_counts <- prepare_csv(path_counts)
filter_counts <- filter_islands(prep_counts, filter_col, filter_value)
```
...
- add a filter argument `filter_gt_adults` (default set to NULL) and when it is not NULL, filter the data only greater than the value provided
...
- add checks to your function `sum_counts()`
- build a plot function that plots the output of `sum_counts()` called `plot_xy()`
- add `x_col` and `y_col` arguments to your plot function to dynamically update plot
- use `x_col` and `y_col` arguments to also update the axis labels in `plot_xy()` (base R)
